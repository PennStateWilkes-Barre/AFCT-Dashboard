...
- **Edit** holds **Undo** and **Redo**, which step back through anything you have changed about the drawing: a state dragged to a new place, the layout switched, a state renamed, an initial or final mark ticked, or what a transition reads. Typing into a box is one step for the whole box rather than one per letter. Zoom and panning are not included, since they move the view rather than the machine.
